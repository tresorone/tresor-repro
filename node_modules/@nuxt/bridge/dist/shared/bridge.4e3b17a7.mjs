import { useNuxt, addPluginTemplate, tryImportModule, resolvePath, resolveFiles, addTemplate, resolveAlias, addWebpackPlugin, addVitePlugin, tryResolveModule, addPlugin, extendWebpackConfig, defineNuxtModule, logger, addComponent, addImportsSources, installModule, writeTypes as writeTypes$1, checkNuxtCompatibility } from '@nuxt/kit';
...
import { dirname, join as join$1, resolve, normalize, isAbsolute, relative, basename, extname, parse as parse$1 } from 'pathe';
...
import { genDynamicImport, genString, genImport, genSafeVariableName, genArrayFromRaw } from 'knitwork';
...
class AsyncLoadingPlugin {
  constructor(opts) {
    __publicField$1(this, "opts");
    __publicField$1(this, "Template");
    this.opts = opts;
...
const schemaTemplate = {
  ...
  getContents: async ({ nuxt }) => {
    ...
    const getImportName = (name) => (name.startsWith(".") ? "./" + join$1(relativeRoot, name) : name).replace(/\.\w+$/, "");
    ...
    const generatedPrivateTypes = generateTypes(
      await resolveSchema(runtimeConfigs.private),
      {
    ...
    const generatedPublicTypes = generateTypes(
      await resolveSchema(runtimeConfigs.public),
      {
...
const globalMiddlewareTemplate = {
...
const PageMetaPlugin = createUnplugin(
  (options) => {
    return {
      ...
      transformInclude(id) {
        const { pathname, search } = parseURL(
          decodeURIComponent(pathToFileURL$1(id).href)
        );
        const { type, macro, setup } = parseQuery(search);
        if (pathname.endsWith(".vue") && (type === "template" || type === "script" || setup === "true" || macro || !search)) {
          return true;
        }
      },
      transform(code, id) {
        ...
        walk(
          ...
          {
            enter(_node) {
              if (!["ExportDefaultDeclaration", "VariableDeclaration"].includes(
                _node.type
              )) {
                return;
              }
              const exportDeclaration = _node;
              ...
              const setupNode = properties.find(
                (node) => node.key && node.key.type === "Identifier" && node.key.name === "setup"
              );
              if (!setupNode) {
                return;
              }
              ...
              walk(setupNode, {
                enter(_node2) {
                  if (_node2.type !== "CallExpression" || _node2.callee.type !== "Identifier") {
                    return;
                  }
                  const node = _node2;
                  ...
                  const metaProperties = meta.properties.map(
                    (prop) => code.slice(prop.start, prop.end) || {}
                  ).join(",");
                  contents = [
...
function getObjectExpression(node) {
  ...
  if (node.type === "CallExpression") {
    ...
    if (node.arguments[0].type === "CallExpression") {
      ...
      return callexpression.arguments.find((arg) => arg.type === "ObjectExpression" && arg.properties.length > 0);
    }
  ...
  if (node.type === "VariableDeclarator") {
    ...
    if (node.init.type === "CallExpression") {
      return node.init.arguments[0];
    }
  }
}
function parseMacroQuery(id) {
  const { search } = parseURL(
    decodeURIComponent(isAbsolute(id) ? pathToFileURL$1(id).href : id)
  );
  return parseQuery(search);
}

const pageMetaModule = defineNuxtModule({
  meta: {
    name: "page-meta",
    configKey: "pageMeta"
  },
  setup(_, nuxt) {
    const pageMetaOptions = {
      sourcemap: true
      // nuxt.options.sourcemap not yet supported
    };
...
const module = defineNuxtModule({
  ...
  async setup(opts, nuxt) {
    ...
    if (opts.nitro) {
      if (!await import('../chunks/features.mjs').then((r) => r.ensurePackageInstalled(nuxt.options.rootDir, "nuxi", nuxt.options.modulesDir))) {
